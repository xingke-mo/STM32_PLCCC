    ...
    while(1)
    {
        ...
        case WorkStatus_Config://主节点配置状态
            ...
            case ConfigStatus_link:
                ...
                {
                    goto TIMEOUTCHECK;    //无数据
                }

                if(read_3762_str(com4.DMA_RX_BUF, com4.lenRec))
                {
                    goto TIMEOUTCHECK;    //校验错
                }

                if(!(readResult.AFN_code == AFN03 && readResult.F_code == F10))
                {
                    goto TIMEOUTCHECK;    //非AFN03F10指令
                }
...
TIMEOUTCHECK:
            ...
            case ConfigStatus_addrinit:// 地址初始化
                ...
                if(!(readResult.AFN_code == AFN00 && readResult.F_code == F1))
                {
                    break;    //非AFN00F1指令
                }
