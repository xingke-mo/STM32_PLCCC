...
u8 checkCard(u8 Id[], u8 key[])
{
    ...
    for(i = 0; i < 18; i++)
    {
        com2.DMA_TX_BUF[i] = CheckKey[i];    //复制基本数据
    }

    for(i = 0; i < 4; i++)
    {
        com2.DMA_TX_BUF[i + 5] = Id[i];    //填写卡片号
    }

    for(i = 0; i < 6; i++)
    {
        com2.DMA_TX_BUF[i + 9] = key[i];    //填写卡片密码
    }
...
u8 readCard(u8 area, u8 data[])
{
    ...
    for(i = 0; i < 7; i++)
    {
        com2.DMA_TX_BUF[i] = ReadCard[i];    //复制基本数据
    }
...
u8 writeCard(u8 area, u8 dat[])
{
    ...
    for(i = 0; i < 23; i++)
    {
        com2.DMA_TX_BUF[i] = WriteCard[i];    //复制基本数据
    }
        ...
        if(checkCard(a, b))
        {
            if(readCard(4, Area4Data)) //读状态
            {
                *c = Area4Data[0]; //卡状态
                ...
                for(i = 0; i < 6; i++)
                {
                    d[i] = Area4Data[i + 1];    //加电时间
                }

                for(i = 0; i < 6; i++)
                {
                    e[i] = Area4Data[i + 7];    //结算时间
                }
